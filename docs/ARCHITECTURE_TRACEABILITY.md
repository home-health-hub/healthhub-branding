...
| 7. MQTT administration separately reports and tests connectivity, authentication, TLS, ACLs, event recovery, and retained state; uses synthetic previews; preserves known-good configuration; and exposes no general publish/subscription/ACL/broker console | Required administration boundary | `Claude-foundation.md` §§25–29; `Preliminary.md` §10 plus owner-approved Web UI design | AR-006 owner decision 2026-08-20 |
...
| 12. Password is primary and user-enabled TOTP is an optional second factor, with confirmed enrollment, QR and selectable-text setup, one-time backup codes, password-plus-second-factor login, and audited SHA last-resort reset that grants no health-data access | Required account-security capability | `Preliminary.md` §12 Account Login Credentials | AR-011 owner clarification 2026-08-20 |
| 12. TOTP secrets are encrypted; backup codes are salted, hashed, and atomically consumed; secret material is excluded from output; verification is rate-limited, replay-resistant, and narrowly clock-tolerant; reset revokes active sessions | Required credential-security boundary | `Preliminary.md` §§12, 24 plus owner-approved safeguards | AR-011 owner decision 2026-08-20 |
...
| 13. Storage administration distinguishes exact/estimated and pressure/loss states, exposes no general filesystem browser, uses fixed component-owned cleanup, never lets the Hub delete daemon readings, previews effects, protects the last copy, and reports audited partial results | Required administration boundary | `Preliminary.md` §§5, 18–19, 23, 25 plus owner-approved Web UI design | AR-002, AR-004, AR-012 owner decision 2026-08-20 |
...
| 14. The Web UI reports trusted update metadata, compatibility and readiness before host-side installation, and migration/health evidence afterward, without accepting arbitrary sources or inferring success from version alone | Required administration interface | `Preliminary.md` §§20–23 plus owner-approved Web UI design | AR-013 owner decision 2026-08-20 |
