...
A password is the primary credential for a Health Hub account. A user may opt in to time-based one-time password (TOTP) authentication as a second factor; it is not required by default. During enrollment, the Hub generates a random secret and presents the `otpauth://totp/` URI both as a QR code and as selectable text so enrollment works when the Hub station is the user's only display. TOTP remains disabled until the user proves possession by entering a valid code. The Hub then generates one-time backup codes, displays them once, and requires the user to acknowledge their storage. After enrollment, login requires the password plus either a valid TOTP code or one unused backup code. Self-service disablement requires the current password and a current TOTP or backup code. If both the authenticator and backup codes are lost, the SHA may perform an audited TOTP reset solely to restore account login; that action grants no health-data access and revokes the account's active sessions. [AR-011; `Preliminary.md` §12]

The TOTP secret is encrypted at rest because verification requires recovery of the secret. Backup codes are independently salted and hashed using the password-storage standard, consumed atomically on successful use, and never recoverable. Enrollment secrets, QR payloads, backup codes, and submitted one-time codes are excluded from URLs, logs, notices, exports, analytics, and diagnostics. Verification is rate-limited, accepts only a narrowly documented clock-skew window, and rejects reuse of a code already accepted for that account and time step. Enrollment, successful enablement, disablement, backup-code use, backup-code regeneration, failed or successful SHA reset, and resulting session revocation produce appropriately data-minimized audit events. [AR-011; `Preliminary.md` §§12, 24]
